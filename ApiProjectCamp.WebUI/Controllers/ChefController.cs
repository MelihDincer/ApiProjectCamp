...
namespace ApiProjectCamp.WebUI.Controllers
{
    public class ChefController : Controller
    {
        ...
        public async Task<IActionResult> ChefList()
        {
            ...
            HttpResponseMessage responseMessage = await client.GetAsync("https://localhost:44338/api/Chefs");
        ...
        public async Task<IActionResult> CreateChef(CreateChefDto createChefDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:44338/api/Chefs", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            HttpClient client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:44338/api/Chefs?id=" + id);
            return RedirectToAction("ChefList");
        }
        ...
        {
            HttpClient client = _httpClientFactory.CreateClient();
            HttpResponseMessage responseMessage = await client.GetAsync("https://localhost:44338/api/Chefs/GetChef?id=" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateChef(UpdateChefDto updateChefDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:44338/api/Chefs", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("ChefList");
            }
