    ...
    public class ChatHub : Hub
    {
        ...
        public override Task OnConnectedAsync()
        {
            _history[Context.ConnectionId] = new List<Dictionary<string, string>> {
                ...
                {
                    ["role"] = "system",
                    ["content"] = "You are a helpful assistan. Keep answers concise."
                }
            };
        ...
        public async Task SendMessage(string userMessage)
        {
            ...
            await StremOpenAI(history, Context.ConnectionAborted);
        }

        public async Task StremOpenAI(List<Dictionary<string, string>> history, CancellationToken cancellationToken)
        {
            ...
            while (!reader.EndOfStream && !cancellationToken.IsCancellationRequested) //okuma işlemi devam ettiği sürece
            {
        ...
        {
            [JsonPropertyName("choies")] public List<Choice>? Choices { get; set; }
        }
